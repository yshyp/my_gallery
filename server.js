...
const uploadImage = multer({
  ...
  fileFilter: (_, file, cb) => {
    const ok = ['image/jpeg','image/png','image/gif','image/webp','image/bmp','image/tiff'].includes(file.mimetype);
...
async function optimizeImage(originalPath, filename) {
  ...
  const optimized = `${base}-optimized${ext}`;
  const thumbnail = `${base}-thumb${ext}`;
  try {
    await sharp(originalPath)
      .resize({ width: 1920, withoutEnlargement: true })
      .jpeg({ quality: 80, progressive: true })
      .toFile(path.join(OPTIMIZED_DIR, optimized));

    await sharp(originalPath)
      .resize({ width: 400, withoutEnlargement: true })
      .jpeg({ quality: 70, progressive: true })
      .toFile(path.join(THUMBNAILS_DIR, thumbnail));
  } catch (e) {
...
app.post('/api/upload', uploadImage.single('image'), async (req, res) => {
  ...
  const { optimized, thumbnail } = await optimizeImage(req.file.path, req.file.filename);
  res.json({
    filename: req.file.filename,
    originalUrl: `/uploads/${req.file.filename}`,
    optimizedUrl: `/optimized/${optimized}`,
...
app.get('/api/images', (_, res) => {
  fs.readdir(UPLOADS_DIR, (err, files) => {
    ...
    const images = files.filter(f => /\.(jpg|jpeg|png|gif|webp|bmp|tiff)$/i.test(f) && !f.includes('-optimized') && !f.includes('-thumb'))
      .map(f => {
        const [stamp] = f.split('-');
...
app.get('/api/videos', (_, res) => {
  fs.readdir(VIDEOS_DIR, (err, files) => {
    ...
    res.json(videos.sort((a, b) => b.timestamp - a.timestamp));
  });
});
